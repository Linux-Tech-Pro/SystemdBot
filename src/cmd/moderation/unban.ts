...
class Ban extends JollyCommand {
    constructor() {
        super("unban", "moderation", {
            ...
            description: "Unban someone. This requires the User's ID (right click on member, select Copy ID)"
        })
    }
    ...
    override async run(message: Message, args: string[], client: BotWithCache<Bot>) {
        ...
        if (isUserID) {
            ...
            return await send(client, message.channelId, `You need to give a user's ID in order to unban.`)
        }
    }
}
