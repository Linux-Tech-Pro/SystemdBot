...
class Ban extends JollyCommand {
    constructor() {
        super("tempban", "moderation", {
            ...
            description: "Same thing as ban, but with Time!",
    ...
    override async run(message: Message, args: string[], client: BotWithCache<Bot>) {
        ...
        if (!time) return send(client, message.channelId, "Please specify the time (Example: 5m, 3h, 2d, 2w...).")
        ...
        return await send(client, message.channelId, `<@${member.id}> has been temporarily banned!`)
    }
}
